...
// It connects to NATS and is ready to receive and forward Falco runtime
...
func main() {
	...
	slog.Info("event bus initialized", "type", "falco", "bus_subject", bus != nil)

	slog.Info("adapter-falco ready, waiting for events", "nats_url", natsURL)

	// In production, this would connect to Falco gRPC and stream events
	// through the event bus. For MVP, it sits ready to receive and forward events.
	<-ctx.Done()
